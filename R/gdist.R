...
#' 
#' gdist() gives the geodesic distance between two points specified by
#' latitude/longitude using Vincenty inverse formula for ellipsoids.  The
#' distance is given in nautical miles (the default), meters, kilometers, or
#' miles. gdist.total() applies gdist() to any number of line segments.
#' 
#' 
#' @aliases gdist gdist.total
#' @param lon.1 Longitude of the first point.
#' @param lat.1 Latitude of the first point.
#' @param lon.2 Longitude of the second point.
#' @param lat.2 Latitude of the second point.
#' @param units Any one of "nm": nautical miles (the default), "m": meters,
#' "km": kilometers, or "miles": miles, for units of the returned value.
#' @param a Major semiaxes of the ellipsoid in meters. The default is for the
#' Earth: 6378137.0
#' @param b Minor semiaxes of the ellipsoid in meters. The default is for the
#' Earth: 6356752.3142
#' @param verbose Should extra information be output, the default is FALSE.
#' @param longlat Two dimensional (n x 2) data in the form of a matrix or
#' dataframe. The first column should be the longitude or x-axis column and the
#' second the latitude or y-axis column.
#' @param segments When there are more than two or more segments, should the
#' distance for each segment be output. The default is TRUE.
#' @param digits The number of rounding digits to use in the segment output.
#' The default is 2.
#' @return The total geodesic distance in the units specified.
#' @author John R. Wallace: \email{Imap.for.R@@gmail.com} (Limited support)
#' @seealso \code{\link{imap}}, \code{\link{draw.lines}}
#' @references Inspired by:
#' \url{http://www.movable-type.co.uk/scripts/LatLongVincenty.html}
#' 
#' Vincenty, T.  Direct and inverse solutions of geodesics on the ellipsoid
#' with application of nested equations.  Survey Review XXII, 176, April 1975.
