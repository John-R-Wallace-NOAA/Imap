 ...
 gdist <- function(lon.1, lat.1, lon.2, lat.2, units = 'nm', a = 6378137.0, b = 6356752.3142, verbose = FALSE) {    
    ...
    " #  End gdist.atomic()   "  
      ...
      out <- rep(NA, length(lon.1))
      for (i in 1:length(lon.1)) {    
            out[i] <- gdist.atomic(lon.1[i], lat.1[i], lon.2[i], lat.2[i], units = units, a = a, b = b, verbose = verbose)
      }  
      out
